...
def update_area_index(map, crime):
    ...
    """
    area= crime.get("REPORTING_AREA", "")
    if area=='' or area==' ' or area is None:
        area_key='9999'
    else:
        area_key=int(area)
    entry = rbt.get(map, area_key)
    if entry is None:
        datoentrada={'lstcrimes':al.new_list()}
        rbt.put(map, area_key, datoentrada)
    else:        datoentrada=entry
    al.add_last(datoentrada['lstcrimes'], crime)
